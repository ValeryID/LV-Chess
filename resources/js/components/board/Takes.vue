...
export default {
    ...
    methods: {
        render() {
            ...
            let white_takes = 0, black_takes = 0
            for(const take of this.takes) {
                const numberColor = take.color === 'w'
                this.renderPiece('pqrbkn'.indexOf(take.type), !numberColor, 
                numberColor ? white_takes++ : black_takes++, numberColor)
            }
        },
        clear() {
        ...
        renderPiece(sx, sy, bx, by) {
            this.ctx.drawImage(Store.state.spriteSheet, 
                sx * 100, sy * 100, 
                100, 100, 
