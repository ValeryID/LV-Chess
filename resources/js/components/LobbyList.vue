        ...
        <div class="slider-div">
            <div v-if="openLobbies().length > 0">
                <template v-for="lobby in lobbies">
                    <button
                        v-if="lobby.status === 'open'"
                        @click="join(lobby.id)"
                        class="lobby-list-item"
                    >
                        {{ `${lobby.id}. (${1 + (lobby.guest !== null)}/2)`
                        }}<br />
...
export default {
    ...
    created() {
        Network.getLobbies()
            .then((lobbies) => (Store.state.lobbies = lobbies))
            .then(() => {
                for (let lobby of Store.state.lobbies) {
                    if (
                        ...
                        [
                            lobby.host.id,
                            lobby.guest ? lobby.guest.id : {},
                        ].includes(Store.state.user.id)
                    )
                        this.join(lobby.id, true);
                }
            });

        Network.listen("LobbyEvent", "created", (event) =>
            Store.state.lobbies.push(event.lobby)
        );
        Network.listen("LobbyEvent", "updated", (event) => {
            Store.state.lobbies = Store.state.lobbies.map((lobby) =>
                lobby.id === event.lobby.id ? event.lobby : lobby
            );
        });
    },
};
